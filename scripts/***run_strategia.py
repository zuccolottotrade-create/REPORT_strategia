...
def load_kpi_csv(path: Path) -> pd.DataFrame:
    ...
    # ------------------------------------------------------------
    # Lettura CSV KPI robusta: alcuni file KPI sono TSV (\t) e non ';'
    # ------------------------------------------------------------
    df = None
    last_err = None
    for _sep in (CSV_SEP, "\t", ","):
        try:
            tmp = pd.read_csv(path, sep=_sep, dtype=str)
            tmp.columns = [c.strip() for c in tmp.columns]

            # Se il separatore è sbagliato spesso resta UNA sola colonna con i separatori nel nome
            if len(tmp.columns) == 1 and any(s in tmp.columns[0] for s in ("\t", ";", ",")):
                continue

            # Accettiamo se c'è 'close' e (date+time) oppure 'datetime'
            if "close" in tmp.columns and (("date" in tmp.columns and "time" in tmp.columns) or ("datetime" in tmp.columns)):
                df = tmp
                break
    ...
    if df is None:
        graceful_exit(
            f"Impossibile leggere CSV KPI: separatori provati {CSV_SEP!r}, \\t, ',' (last_err={last_err})",
            2,
        )

    # --- datetime robustness: accetta KPI con sola colonna 'datetime' (senza date/time) ---
...
def apply_hold_value(
    ...
    *,
    ...
    # Se VALUE_no_regime è NaN sulle entry, la valorizziamo con close della barra di entry (robusto e coerente con apply_hold_value)
    out.loc[m_entry_nr & out["VALUE_no_regime"].isna(), "VALUE_no_regime"] = out.loc[
        m_entry_nr & out["VALUE_no_regime"].isna(), "close"
    ]
...
def main() -> None:
    ...
    if all(c in df.columns for c in required_nr):
        ...
        m_in = df_tmp["HOLD_no_regime"].astype(str).str.upper().eq("IN")
        df_tmp.loc[m_in & df_tmp["VALUE_no_regime"].isna(), "VALUE_no_regime"] = df_tmp.loc[
            m_in & df_tmp["VALUE_no_regime"].isna(), "close"
        ]

        # riallineo VALUE "atteso" dopo il fill
        df_tmp.loc[m_in & df_tmp["VALUE"].isna(), "VALUE"] = df_tmp.loc[m_in & df_tmp["VALUE"].isna(), "close"]
